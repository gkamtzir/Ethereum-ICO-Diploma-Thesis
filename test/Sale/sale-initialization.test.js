...
contract("Sale -> initialize", accounts => {
    ...
    describe("Initialization", () => {
        ...
        it("Should initialize the sale with the correct owner", async () => {
            const owner = await this.privateSale.getOwner();
            expect(owner).to.be.equal(this.admin);
        });

        it("Should initialize the sale with the correct price", async () => {
            const price = await this.privateSale.getTokenPrice();
            expect(price).to.be.bignumber.equal(this.tokenPrice);
        });

        it("Should initialize the sale with the correct total supply", async () => {
        ...
        it("Should initialize the sale with the zero sold tokens", async () => {
